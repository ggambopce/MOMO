...
public class UsrReactionPointController {
	...
	public String doInsertReactionPoint(int relId, String relTypeCode, int point) {
		...
		if (point == 1) {
			return Util.jsReplace(Util.f("%d번 글에 좋아요", relId), Util.f("../mmaker/detail?id=%d", relId));
		} else {
			return Util.jsReplace(Util.f("%d번 글에 싫어요", relId), Util.f("../mmaker/detail?id=%d", relId));
		}
	}
	...
	public String doDeleteReactionPoint(int relId, String relTypeCode, int point) {
		...
		if (point == 1) {
			return Util.jsReplace(Util.f("%d번 글에 좋아요 취소", relId), Util.f("../mmaker/detail?id=%d", relId));
		} else {
			return Util.jsReplace(Util.f("%d번 글에 싫어요 취소", relId), Util.f("../mmaker/detail?id=%d", relId));
		}
	}
